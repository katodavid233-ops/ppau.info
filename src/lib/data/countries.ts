...
export const DEFAULT_DIAL_CODE = "+256";

/** Uganda first, then all others A–Z */
const COUNTRY_ROWS: { name: string; iso2: string; dial: string }[] = [
...
const uganda = COUNTRY_ROWS[0]!;
const rest = [...COUNTRY_ROWS.slice(1)].sort((a, b) => a.name.localeCompare(b.name));

export const COUNTRIES: CountryOption[] = [uganda, ...rest].map((c) => ({
  name: c.name,
  iso2: c.iso2,
  dialCode: c.dial,
}));
...
export function getDialCodeByCountry(name: string): string {
  return getCountryByName(name)?.dialCode ?? DEFAULT_DIAL_CODE;
}

/** Longest dial codes first for parsing */
const DIAL_CODES_DESC = [...new Set(COUNTRIES.map((c) => c.dialCode))].sort(
  (a, b) => b.length - a.length,
);

export function parsePhoneNumber(full: string): { dialCode: string; local: string } {
  const trimmed = full.trim();
  if (!trimmed) return { dialCode: DEFAULT_DIAL_CODE, local: "" };

  for (const dial of DIAL_CODES_DESC) {
    if (trimmed.startsWith(dial)) {
      return { dialCode: dial, local: trimmed.slice(dial.length).trim() };
    }
  }

  const digits = trimmed.replace(/\D/g, "");
  if (digits.startsWith("256")) {
    return { dialCode: "+256", local: digits.slice(3) };
  }

  return { dialCode: DEFAULT_DIAL_CODE, local: trimmed };
}

export function formatPhoneNumber(dialCode: string, local: string): string {
  let digits = local.replace(/\D/g, "");
  if (!digits) return dialCode;

  if (dialCode === "+256" && digits.startsWith("0")) {
